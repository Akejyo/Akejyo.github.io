...
稍微多瞅瞅，在 `PipeCollertorScript` 的里还可以看到管道的生成逻辑：在 `Awake()` 初始化，`OnTriggerEnter2D` 里会让下一个管道生成的 $x$ 坐标增加一个 `distance`，$y$ 坐标在 `pipeMin` 和 `pipeMax` 中间随机选一个，其中 `distance = 3f`， `pipeMax = 2.3f`，`pipeMin = -1f`
