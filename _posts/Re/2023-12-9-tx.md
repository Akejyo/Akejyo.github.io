...
void CreateRemoteThread_inject() {
    ...
    if (FindProcess(L"crackme.exe", dwProcess)) {
        ...
        // LoadLibraryA 函数在目标进程中的地址可能与在这里的进程中的地址不同，因此需要在目标进程中查找 LoadLibraryA 函数的地址，然后作为 CreateRemoteThread 函数的参数
...
原理在代码注释里有点介绍，需要注意的是，主函数生成的 exe 和 `dllmain.cpp` 生成 dll 都必须和 `crackme.exe ` 的平台保持一致，即都是64位。比较坑的是 vs 默认是 32位，被这个坑了巨久。
...
那就很明显了，理论上我直接把18e7和18e9这两个换nop就行
...
然后再利用刚才在1里写的dll注入，直接重写一下dllmain：
